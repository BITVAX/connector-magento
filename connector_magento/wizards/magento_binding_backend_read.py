...
class MagentoBindingBackendRead(models.TransientModel):
    ...
    def _default_magento_backend_id(self):
        ...
        if active_model and active_model != "magento.backend":
            raise UserError(_("The wizard must be launched from a magento backend model"))
        active_id = self._context.get("active_ids", [])
        if len(active_id) > 1:
            raise UserError(_("The wizard must be launched on a single magento backend instance"))
    ...
    magento_binding_model = fields.Selection("_get_magento_binding_model", required=True)
