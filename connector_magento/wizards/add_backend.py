...
class WizardModel(models.TransientModel):
    ...
    def _validate_backend_ready(self):
        ...
        if not self.env["magento.product.attribute.set"].search(
        ...
        for wizard in self:
            ...
            if active_model == "product.template":
                ...
                if template.has_variant_attributes:
                    ...
                    wizard.detected_template_type = (
                        "Configurable Template (has variant attributes)"
                    )
                else:
                    wizard.product_type = "simple"
                    wizard.detected_template_type = (
                        "Simple Template (no variant attributes)"
                    )
                ...
                if template.has_variant_attributes:
                    ...
                    if wizard.product_links:
                        ...
                        wizard.detected_template_type = (
                            "Grouped Product (simple product with links)"
                        )
                    else:
                        wizard.product_type = "simple"
                        wizard.detected_template_type = (
                            "Simple Product (add linked products for Grouped)"
                        )
    ...
    def _compute_warnings(self):
        ...
        for wizard in self:
            ...
            if (
                wizard.detected_template_type
                and "ERROR" in wizard.detected_template_type
            ):
            ...
            if wizard.product_type == "grouped" and not wizard.product_links:
                warnings.append(
                    "⚠ Grouped products require at least one linked product"
                )
            ...
            if wizard.product_links and wizard.backend_id:
                for link in wizard.product_links:
                    ...
                    if not link_bindings:
                        warnings.append(
                            f"⚠ Linked product '{link.display_name}' is not configured for backend '{wizard.backend_id.name}'"
                        )
            ...
            if active_model == "product.template" and active_ids:
                ...
                if template.has_variant_attributes:
                    ...
                    if (
                        not template.code_prefix
                        and not template.product_variant_ids[0].default_code
                    ):
                        warnings.append(
            ...
            if wizard.backend_id:
                if active_model == "product.template" and active_ids:
                    ...
                    if existing:
                        warnings.append(
                            f"⚠ Backend '{wizard.backend_id.name}' is already configured for this template"
                        )
                    ...
                    if existing:
                        warnings.append(
                            f"⚠ Backend '{wizard.backend_id.name}' is already configured for this product"
                        )
    ...
    def _check_grouped_requirements(self):
        ...
        for wizard in self:
            if wizard.product_type == "grouped" and not wizard.product_links:
                raise UserError(
                    _("Grouped products require at least one linked product.")
                )
    ...
    def _process_grouped_product_binding(self):
        ...
        for product in products:
            ...
            existing_binding = self.env["magento.product.product"].search(
                [("odoo_id", "=", product.id), ("backend_id", "=", self.backend_id.id)]
            )
    ...
    def _create_template_binding(self, template):
        ...
        existing_binding = self.env["magento.product.template"].search(
            [("odoo_id", "=", template.id), ("backend_id", "=", self.backend_id.id)]
        )
    ...
    action = fields.Selection(
        ...
        default="only_create",
        required=True,
        help="Action to perform after creating binding",
    )
