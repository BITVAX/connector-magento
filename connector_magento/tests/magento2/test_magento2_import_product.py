...
class TestImportProduct(Magento2SyncTestCase):
    ...
    def test_import_product_category_missing(self):
        ...
        backend_id = self.backend.id
        self.assertEqual(
            len(self.env["magento.product.category"].search([("backend_id", "=", backend_id)])), 0
        )
        ...
        self.assertEqual(
            len(self.env["magento.product.category"].search([("backend_id", "=", backend_id)])), 6
        )
    ...
    def test_import_product_grouped(self):
        ...
        with self.assertRaises(InvalidDataError):
            self.env["magento.product.product"].import_record(self.backend, "24-WG085_Group")
