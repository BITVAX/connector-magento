...
class TestConcurrentSync(Magento2TestCase):
    ...
    def test_concurrent_import(self):
        ...
        work = WorkContext(
            model_name="magento.product.category", collection=self.backend, magento_api=api_client
        )
        importer = work.component_by_name("magento.product.category.importer")
        importer.run(1)

        work2 = WorkContext(
            model_name="magento.product.category", collection=self.backend2, magento_api=api_client
        )
