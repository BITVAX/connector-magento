...
ExpectedOrderLine = namedtuple("ExpectedOrderLine", "product_id name price_unit product_uom_qty")
...
class TestSaleOrder(Magento2SyncTestCase):
    ...
    def _import_sale_order(self, increment_id, cassette=True):
        return self._import_record("magento.sale.order", increment_id, cassette=cassette)
    ...
    def test_import_sale_order(self):
        ...
        self.assertEqual(
            ...
            self.workflow,
            "If the automatic workflow is empty, the onchanges have not been applied.",
        )
    ...
    def test_import_sale_order_guest(self):
        ...
        self.assertEqual(
            binding.partner_id.category_id, self.env.ref("connector_magento.category_no_account")
        )
    ...
    def test_import_sale_order_carrier_product(self):
        ...
        self.assertTrue(
            ...
            % (
                ", ".join(
                    "%s (%s)" % (line.name, line.product_id.name) for line in binding.order_line
                ),
            ),
        )
    ...
    def test_import_sale_order_options(self):
        ...
        account_analytic_id = self.env["account.analytic.account"].create({"name": "aaa1"})
        ...
        account_analytic_id = self.env["account.analytic.account"].create({"name": "aaa2"})
        ...
        account_analytic_id = self.env["account.analytic.account"].create({"name": "aaa3"})
        ...
        account_analytic_id = self.env["account.analytic.account"].create({"name": "aaa4"})
    ...
    def test_cancel_export(self):
        ...
        with recorder.use_cassette("test_sale_order_cancel_export") as cassette:
            ...
            self.assertEqual(cassette.requests[1].uri, "http://magento/index.php/rest/V1/orders")
            self.assertEqual(
                cassette.requests[2].uri, "http://magento/index.php/rest/V1/orders/12/comments"
            )
    ...
    def test_copy_quotation_export_state(self):
        ...
        self.assertEqual(cassette.requests[0].uri, "http://magento/index.php/rest/V1/orders/12")
        self.assertEqual(cassette.requests[1].uri, "http://magento/index.php/rest/V1/orders")
        self.assertEqual(
            cassette.requests[2].uri, "http://magento/index.php/rest/V1/orders/12/comments"
        )
        self.assertEqual(cassette.requests[3].uri, "http://magento/index.php/rest/V1/orders/12")
        self.assertEqual(cassette.requests[4].uri, "http://magento/index.php/rest/V1/orders")
        self.assertEqual(
            cassette.requests[5].uri, "http://magento/index.php/rest/V1/orders/12/comments"
        )
