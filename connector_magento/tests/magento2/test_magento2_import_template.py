...
class TestImportProductTemplate(Magento2SyncTestCase):
    ...
    def _import_template(self, sku):
        ...
        return self._import_record("magento.product.template", sku, cassette=True)
    ...
    def test_import_template_configurable(self):
        ...
        self.assertIn(
            "Color", attr_names, "Color attribute line should exist on template"
        )
        self.assertIn(
            "Size", attr_names, "Size attribute line should exist on template"
        )
        ...
        for variant in variants:
            self.assertEqual(
                variant.odoo_id.product_tmpl_id,
                template,
    ...
    def test_import_template_binding_data(self):
        ...
        self.assertEqual(binding.external_id, "CONF-TEST")
        self.assertTrue(binding.data, "Template binding should store magento data")
