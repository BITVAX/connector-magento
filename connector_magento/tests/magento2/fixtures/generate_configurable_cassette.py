...
VARIANT_1 = make_simple_variant(
    "CONF-TEST-S-Red", 1001, "Test Configurable - S - Red", 49, 167, "conf-test-s-red"
)

VARIANT_2 = make_simple_variant(
    "CONF-TEST-M-Blue",
    1002,
    "Test Configurable - M - Blue",
    50,
    168,
    "conf-test-m-blue",
)
...
ATTRIBUTES = {
    ...
    "required_options": make_attr(
        "required_options", 102, "Required Options", "boolean"
    ),
    "description": make_attr("description", 75, "Description", "textarea"),
    "short_description": make_attr(
        "short_description", 76, "Short Description", "textarea"
    ),
    ...
    "options_container": make_attr(
        "options_container", 96, "Display Product Options In", "select"
    ),
    "msrp_display_actual_price_type": make_attr(
        "msrp_display_actual_price_type", 121, "Display Actual Price", "select"
    ),
...
def build_interactions():
    ...
    # 6. Tax classes
    add(
        f"{BASE}/rest/V1/taxClasses/search?fields=items%5Bclass_id%5D&searchCriteria=",
        TAX_SEARCH,
    )
    for prefix in ["/rest/V1", "/rest/all/V1"]:
    ...
    add(
        f"{BASE}/rest/V1/configurable-products/CONF-TEST/children",
        [VARIANT_1, VARIANT_2],
    )
...
def write_cassette(output_path, interactions):
    ...
    print(
        f"    {unique} unique interactions, {total} total (x{DUPLICATE_COUNT} duplicates)"
    )
...
def build_export_product_interactions():
    ...
    def add(uri, data, method="GET"):
        interactions.append(
            interaction(uri, json.dumps(data, separators=(",", ":")), method)
        )
    ...
    add(
        f"{BASE}/rest/V1/stockItems/TEST-EXPORT-SIMPLE",
        {
            "item_id": 5001,
            "product_id": 5001,
            "stock_id": 1,
    ...
    add(f"{BASE}/rest/V1/products/TEST-EXPORT-SIMPLE/stockItems/5001", 5001, "PUT")
