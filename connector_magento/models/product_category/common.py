        ...
        for category in self:
            res.append(
                (
                    category.id,
                    "/".join(
                        category.parents_and_self.mapped(lambda x: x.name if x.parent_id else "")
                    ),
                )
            )
        return res
...
class ProductCategoryAdapter(Component):
    ...
    def search(self, filters=None, from_date=None, to_date=None):
        ...
        if self.collection.version == "1.7":
            return self._call("oerp_catalog_category.search", [filters] if filters else [{}])
    ...
    def read(self, external_id, attributes=None, storeview=None, **kwargs):
        ...
        if self.collection.version == "1.7":
            return self._call(
                "%s.info" % self._magento_model, [int(external_id), storeview, attributes]
            )
    ...
    def tree(self, parent_id=None, storeview_id=None):
        ...
        if self.collection.version == "1.7":
            if parent_id:
                ...
                tree = self._call("%s.tree" % self._magento_model, [parent_id, storeview_id])
    ...
    def assign_product(self, categ_id, product_id, position=0):
        if self.collection.version == "1.7":
            return self._call(
                "%s.assignProduct" % self._magento_model, [categ_id, product_id, position, "id"]
            )
        raise NotImplementedError  # TODO
    ...
    def update_product(self, categ_id, product_id, position=0):
        if self.collection.version == "1.7":
            return self._call(
                "%s.updateProduct" % self._magento_model, [categ_id, product_id, position, "id"]
            )
        raise NotImplementedError  # TODO
