...
def _get_sku(binding):
    ...
    return (
        binding.default_code or (binding.odoo_id and binding.odoo_id.default_code) or ""
    )
    ...
    def on_record_unlink(self, record):
        ...
        with record.backend_id.work_on(record._name) as work:
            ...
            if external_id:
                record.with_delay(
                    identity_key=identity_exact,
                    description=_("Delete product %s from Magento")
                    % (_get_sku(record) or external_id),
    ...
    def update_products(self, record):
        if record.applied_on == "1_product":
            for binding in record.product_tmpl_id.magento_template_bind_ids:
                ...
                binding.with_delay(
                    identity_key=identity_exact,
                    description=_("Export template %s (pricelist change) to Magento")
                    % tmpl_name,
                ).export_record(binding.backend_id)
                for variant in record.product_tmpl_id.product_variant_ids:
                    for vbinding in variant.magento_bind_ids:
                        vbinding.with_delay(
                            identity_key=identity_exact,
                            description=_(
                                "Export product %s (pricelist change) to Magento"
                            )
                            % _get_sku(vbinding),
            ...
            for binding in record.product_id.magento_bind_ids:
                binding.with_delay(
                    identity_key=identity_exact,
                    description=_("Export product %s (pricelist change) to Magento")
                    % _get_sku(binding),
                ).export_record(binding.backend_id)
