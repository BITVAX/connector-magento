...
class MagentoProductProduct(models.Model):
    ...
    odoo_id = fields.Many2one(
        comodel_name="product.product", string="Odoo Product", required=True, ondelete="cascade"
    )
    ...
    def _get_admin_path(self, backend, external_id):
        ...
        magento_internal_id = self.search(
            [("backend_id", "=", backend.id), ("external_id", "=", external_id)], limit=1
        ).magento_internal_id
        if magento_internal_id:
            return "catalog/product/edit/id/%s" % magento_internal_id
        raise UserError(
            _("We have to import the product before we can provide the admin link to it.")
        )
...
class ProductProductAdapter(Component):
    ...
    def read(self, external_id, storeview=None, attributes=None, **kwargs):
        ...
        if self.collection.version == "1.7":
            return self._call(
                "ol_catalog_product.info", [int(external_id), storeview, attributes, "id"]
            )
    ...
    def write(self, external_id, data, storeview=None, **kwargs):
        ...
        if self.collection.version == "1.7":
            return self._call(
                "ol_catalog_product.update", [int(external_id), data, storeview, "id"]
            )
    ...
    def get_images(self, external_id, storeview_id=None, data=None):
        ...
        if self.collection.version == "1.7":
            return self._call("product_media.list", [int(external_id), storeview_id, "id"])
        ...
        base_url = storeview.base_media_url or "%s/media/" % self.backend_record.location
    ...
    def update_inventory(self, external_id, data):
        ...
        if self.collection.version == "1.7":
            ...
            return self._call("oerp_cataloginventory_stock_item.update", [int(external_id), data])
        ...
        for item in res:
            ...
            raise ValueError(
                "No stock item found for product %s for default stock_id 1" % external_id
            )
        self._call(
