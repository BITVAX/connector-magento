...
class ProductBatchImporter(Component):
    ...
    def run(self, filters=None):
        ...
        external_ids = self.backend_adapter.search(filters, from_date=from_date, to_date=to_date)
        _logger.info("search for magento products %s returned %s", filters, external_ids)
...
class CatalogImageImporter(Component):
    ...
    def _get_images(self, storeview_id=None, data=None):
        return self.backend_adapter.get_images(self.external_id, storeview_id, data=data)
    ...
    def _get_binary_image(self, image_data):
        url = image_data["url"]
        headers = {}
        if self.backend_record.auth_basic_username and self.backend_record.auth_basic_password:
        ...
        request = requests.get(
            url, headers=headers, verify=self.backend_record.verify_ssl, timeout=60
        )
...
class ProductImportMapper(Component):
    ...
    def tax_class_id(self, record):
        ...
        tax_attribute = [
            a for a in record["custom_attributes"] if a["attribute_code"] == "tax_class_id"
        ]
        ...
        if not mtax:
            raise MappingError(
                "The tax class with the id %s is not imported." % tax_attribute[0]["value"]
            )
        if not mtax.odoo_id:
            raise MappingError(
    ...
    def product_links(self, record):
        ...
        for link in sorted(record["product_links"], key=lambda x: x["position"]):
            ...
            if not product:
                raise MappingError("The product with sku %s is not imported." % link["sku"])
    ...
    def attributes(self, record):
        ...
        for attribute in record["custom_attributes"]:
            mattribute = attribute_binder.to_internal(
                attribute["attribute_code"], unwrap=False, external_field="attribute_code"
            )
            if mattribute:
                ...
                if not mattribute.is_user_defined or mattribute.frontend_input != "select":
                    continue
                mvalue = value_binder.to_internal(
                    "%s_%s" % (mattribute.attribute_id, str(attribute["value"])), unwrap=False
                )
                if not mvalue:
                    raise MappingError(
            ...
            if set(value_ids) != set(binding.attribute_line_ids.mapped("value_ids").ids):
                ...
                if len(changes):
                    for key, value in changes.items():
                        if value["line_id"]:
                            data["attribute_line_ids"].append(
                                (1, value["line_id"], {"value_ids": [(6, 0, value["new"])]})
                            )
                        else:
                            data["attribute_line_ids"].append(
                                (0, 0, {"attribute_id": key, "value_ids": [(6, 0, value["new"])]})
                            )
...
class ProductImporter(Component):
    ...
    def _import_bundle_dependencies(self):
        ...
        if self.collection.version == "1.7":
            for dependency in [
                ...
                self._import_dependency(dependency["product_id"], "magento.product.product")
        else:
            for dependency in [
                product_link
                for option in self.magento_record["extension_attributes"]["bundle_product_options"]
    ...
    def _import_dependencies(self, **kwargs):
        ...
        self._import_dependency(record["attribute_set_id"], "magento.product.attribute.set")
        # Check and import attributes and values if they do not exist
        self._import_attributes(record)
        for mag_category_id in record.get("category_ids") or record.get("categories", []):
        ...
        if record["type_id"] == "grouped":
            for child in record["product_links"]:
                self._import_dependency(child["linked_product_sku"], "magento.product.product")
    ...
    def _validate_product_type(self, data):
        ...
        if product_type not in available_types:
            raise InvalidDataError(
                "The product type '%s' is not yet supported in the connector." % product_type
            )
