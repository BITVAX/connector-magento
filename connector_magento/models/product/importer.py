...
class CatalogImageImporter(Component):
    ...
    def run(self, external_id, binding, data=None):
        ...
        if len(images):
            c = 0
            for image_data in [
                i for i in images if not int(i.get("disabled", i.get("exclude", 0)))
            ]:
...
class ProductImportMapper(Component):
    ...
    def tax_class_id(self, record):
        ...
        if not tax_attribute:
            return {}
        self.binder_for("magento.account.tax")
    ...
    def attributes(self, record):
        ...
        for attribute in record["custom_attributes"]:
            ...
            if mattribute:
                ...
                if (
                    not mattribute.is_user_defined
...
class ProductImporter(Component):
    ...
    def _create(self, data, **kwargs):
        if "binding_template_id" in kwargs:
            ...
            if filtered_fields:
                _logger.info(
                    "ProductImporter._create: Filtered fields for variant: %s",
                    filtered_fields,
                )
    ...
    def _update(self, binding, data, **kwargs):
        ...
        if "binding_template_id" in kwargs:
            ...
            if filtered_fields:
                _logger.info(
                    "ProductImporter._update: Filtered fields for variant %s: %s",
                    binding.external_id,
                    filtered_fields,
                )
