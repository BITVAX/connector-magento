...
class ProductAttributeExporter(Component):
    ...
    def _update_attribute_with_result(self, result):
        ...
        for option in result["options"]:
            ...
            option_binding = self.binding.magento_attribute_value_ids.filtered(
                lambda b: b.code == option["value"]
            )
            # Second try - match on label - with no code set already
            if not option_binding:
                option_binding = self.binding.magento_attribute_value_ids.filtered(
                    lambda b: b.name == option["label"] and not b.code
                )
            ...
            if option_binding:
                option_binding.with_context(connector_no_export=True).write(
                    {
                        "code": option["value"],
                        "external_id": "%s_%s"
                        % (str(result["attribute_id"]), str(option["value"])),
                    }
                )
            else:
                ...
                if odoo_option:
                    # Create binding here
                    external_id = "%s_%s" % (
                        str(result["attribute_id"]),
                        tools.ustr(option["value"]),
                    )
...
class ProductAttributeExportMapper(Component):
    ...
    def value_options(self, record):
        ...
        value_mapper = self.component(usage="export.mapper", model_name="product.attribute.value")
