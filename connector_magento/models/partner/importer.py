...
class PartnerImportMapper(Component):
    ...
    def names(self, record):
        ...
        parts = [
            part
            for part in (record["firstname"], record.get("middlename"), record["lastname"])
            if part
        ]
    ...
    def customer_group_id(self, record):
        ...
        if not category:
            raise MappingError(
                "The partner category with magento id %s does not exist" % record["group_id"]
            )
...
class PartnerAddressBook(Component):
    ...
    def _read_addresses(self, magento_partner_id):
        ...
        if self.collection.version == "1.7":
            ...
            return [(address_id, adapter.read(address_id)) for address_id in mag_address_ids]
    ...
    def _get_address_infos(self, magento_partner_id, partner_binding_id):
        for address_id, magento_record in self._read_addresses(magento_partner_id):
            ...
            address_infos = AddressInfos(
                magento_record=magento_record, partner_binding_id=partner_binding_id, merge=merge
            )
...
class BaseAddressImportMapper(AbstractComponent):
    ...
    def title(self, record):
        ...
        if not prefix:
            return
        title = self.env["res.partner.title"].search([("shortcut", "=ilike", prefix)], limit=1)
        if not title:
...
class AddressImporter(Component):
    ...
    def _define_partner_relationship(self, data):
        ...
        assert partner_binding_id, "AdressInfos are required for creation of a new address."
