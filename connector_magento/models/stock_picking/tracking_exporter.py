...
class MagentoTrackingExporter(Component):
    ...
    def _validate(self, binding):
        if binding.state != "done":  # should not happen
            raise ValueError(
                "Wrong value for picking state, it must be 'done', found: %s" % binding.state
            )
    ...
    def run(self, binding):
        ...
        carrier = binding.carrier_id
        if not carrier:
            return FailedJobError("The carrier is missing on the picking %s." % binding.name)

        if not carrier.magento_export_tracking:
            return _("The carrier %s does not export tracking numbers.") % carrier.name
