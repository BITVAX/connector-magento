...
class MagentoStockPicking(models.Model):
    ...
    odoo_id = fields.Many2one(
        comodel_name="stock.picking", string="Stock Picking", required=True, ondelete="cascade"
    )
    magento_order_id = fields.Many2one(
        comodel_name="magento.sale.order", string="Magento Sale Order", ondelete="set null"
    )
...
class StockPickingAdapter(Component):
    ...
    def create(self, order_id, items, comment, email, include_comment):
        ...
        return self._call(
            "%s.create" % self._magento_model, [order_id, items, comment, email, include_comment]
        )
