...
class MagentoSaleOrder(models.Model):
    ...
    odoo_id = fields.Many2one(
        comodel_name="sale.order", string="Sale Order", required=True, ondelete="cascade"
    )
    ...
    storeview_id = fields.Many2one(comodel_name="magento.storeview", string="Magento Storeview")
    store_id = fields.Many2one(related="storeview_id.store_id", string="Storeview", readonly=True)
...
class MagentoSaleOrderLine(models.Model):
    ...
    odoo_id = fields.Many2one(
        comodel_name="sale.order.line", string="Sale Order Line", required=True, ondelete="cascade"
    )
...
class SaleOrderAdapter(Component):
    ...
    def read(self, external_id, attributes=None):
        ...
        if self.collection.version == "1.7":
            return self._call("%s.info" % self._magento_model, [external_id, attributes])
    ...
    def add_comment(self, external_id, status, comment=None, notify=False):
        return self._call(
            "%s.addComment" % self._magento_model, [external_id, status, comment, notify]
        )
