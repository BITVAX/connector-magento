...
class MagentoTaxClass(models.Model):
    ...
    odoo_id = fields.Many2one(
        comodel_name="account.tax", string="Tax", required=True, ondelete="restrict"
    )
    product_tax_ids = fields.Many2many(
        "account.tax",
        "product_tax_rel",
        string="Product Sale Tax Ids",
        domain=[("type_tax_use", "=", "sale")],
    )
    product_tax_purchase_ids = fields.Many2many(
        "account.tax",
        "product_tax_purchase_rel",
        string="Product Purchase Tax Ids",
