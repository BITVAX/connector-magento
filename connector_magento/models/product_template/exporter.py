...
class ProductTemplateDefinitionExporter(Component):
    ...
    def _update_binding_record_after_create(self, data):
        ...
        importer = self.component(usage="record.importer", model_name="magento.product.template")
            ...
            if self._must_update_variants() or created or not m_prod.external_id:
                if created or not m_prod.external_id:
                    ...
                    delayed = m_prod.with_delay(
                        identity_key=("magento_product_product_%s" % m_prod.id), priority=5
                    ).sync_to_magento()
    ...
    def _create_attribute_lines(self):
        ...
        for line in record.attribute_line_ids:
            ...
            if not m_att_id:
                raise MappingError(
                    "The product attribute %s is not exported yet." % line.attribute_id.name
                )
...
class ProductTemplateExportMapper(Component):
    ...
    def configurable_product_options(self, record):
        ...
        for l in att_lines:
            ...
            if not m_att_id:
                raise MappingError(
                    "The product attribute %s is not exported yet." % l.attribute_id.name
                )
            opt = {
                "id": 0,
