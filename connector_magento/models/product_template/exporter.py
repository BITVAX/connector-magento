...
class ProductTemplateDefinitionExporter(Component):
    ...
    def _update_binding_record_after_create(self, data):
        ...
        if self.backend_record.product_synchro_strategy == "odoo_first":
            mapper = self.component(
                usage="record.update.create", model_name="magento.product.template"
            )
        ...
        importer = self.component(
            usage="record.importer", model_name="magento.product.template"
        )
    ...
    def _update_binding_record_after_write(self, data):
        ...
        if self.backend_record.product_synchro_strategy == "odoo_first":
            mapper = self.component(
                usage="record.update.write", model_name="magento.product.template"
            )
            map_record = mapper.map_record(data)
            update_data = map_record.values(binding=self.binding)
            _logger.info(
                "Got Update data: %s for binding %s", update_data, self.binding
            )
            self.binding.with_context(connector_no_export=True).write(update_data)
    ...
    def _export_variants(self):
        record = self.binding
        variant_exporter = self.component(
            usage="record.exporter", model_name="magento.product.product"
        )
        for p in record.product_variant_ids:
            m_prod = p.magento_bind_ids.filtered(
    ...
    def _create_attribute_lines(self):
        ...
        for line in record.attribute_line_ids:
            m_line = line.magento_bind_ids.filtered(
                lambda m: m.backend_id == record.backend_id
            )
            m_att_id = line.attribute_id.magento_bind_ids.filtered(
                lambda m: m.backend_id == record.backend_id
            )
            if not m_att_id:
                raise MappingError(
                    "The product attribute %s "
                    "is not exported yet." % line.attribute_id.name
                )
...
class ProductTemplateExportMapper(Component):
    ...
    def configurable_product_links(self, record):
        ...
        att_lines = record.attribute_line_ids.filtered(
            lambda l: (
                l.attribute_id.create_variant != "no_variant"
                and len(
                    l.attribute_id.magento_bind_ids.filtered(
                        lambda m: m.backend_id == record.backend_id
                    )
                )
                > 0
    ...
    def configurable_product_options(self, record):
        ...
        att_lines = record.attribute_line_ids.filtered(
            lambda l: (
                l.attribute_id.create_variant in ["always", "dynamic"]
                and len(
                    l.attribute_id.magento_bind_ids.filtered(
                        lambda m: m.backend_id == record.backend_id
                    )
        ...
        for l in att_lines:
            ...
            for v in l.value_ids:
                ...
                for v_id in v_ids:
                    opt["values"].append(
                        {"value_index": v_id.external_id.split("_")[1]}
                    )
    ...
    def get_non_configurable_attributes(self, record):
        ...
        for line in record.attribute_line_ids:
            if (
                ...
                or len(line.value_ids) > 1
                or not line.value_ids
            ):
                continue
            m_att_id = line.attribute_id.magento_bind_ids.filtered(
                lambda m: m.backend_id == record.backend_id
            ...
            if not v_ids:
                raise MappingError(
                    "The product attribute value %s is not exported yet." % v.name
                )
    ...
    def get_custom_attributes(self, record):
        ...
        if record.magento_url_key:
            custom_attributes.append(
                {"attribute_code": "url_key", "value": record.magento_url_key}
            )

        # Add meta fields as custom attributes
        if record.odoo_id.meta_title:
            custom_attributes.append(
                {"attribute_code": "meta_title", "value": record.odoo_id.meta_title}
            )

        if record.odoo_id.meta_keyword:
            custom_attributes.append(
                {"attribute_code": "meta_keyword", "value": record.odoo_id.meta_keyword}
            )
        ...
        if tax_class_value:
            custom_attributes.append(
                {"attribute_code": "tax_class_id", "value": tax_class_value}
            )
