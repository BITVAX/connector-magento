...
class MagentoLocation(object):
    def __init__(
        self, location, username, password, token, version, verify_ssl, use_custom_api_path=False
    ):
...
class MagentoAPI(object):
    ...
    def api_call(self, method, arguments, http_method=None, storeview=None):
        ...
        return self.api.call(method, arguments, http_method=http_method, storeview=storeview)
    ...
    def call(self, method, arguments, http_method=None, storeview=None):
        try:
            ...
            raise NetworkRetryableError("A network error caused the failure of the job: %s" % err)
...
class GenericAdapter(AbstractComponent):
    ...
    def search(self, filters=None):
        ...
        if self.collection.version == "1.7":
            return self._call("%s.search" % self._magento_model, [filters] if filters else [{}])
    ...
    def read(self, external_id, attributes=None, storeview=None, **kwargs):
        ...
        if self.collection.version == "1.7":
            ...
            return self._call("%s.info" % self._magento_model, arguments, storeview=storeview)
    ...
    def create(self, data, storeview=None, **kwargs):
        ...
        if self.work.magento_api._location.version == "2.0":
            if self._magento2_name:
                ...
                new_object = self._call(self._magento2_model % kwargs, data, http_method="post")
    ...
    def delete(self, external_id, **kwargs):
        ...
        res = self._call(
            "%s/%s" % (self._magento2_model, self.escape(external_id)), None, http_method="delete"
        )
